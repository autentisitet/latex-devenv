...
* **Focused Toolchain Orchestration** – Windows CI validates the Scoop/MiKTeX path, while Debian/Ubuntu and the canonical PDF build use the apt-based Podman/TeX Live environment.
...
* **Dual-platform verification:** Every template must compile successfully with Windows MiKTeX and Linux TeX Live.

* **Canonical output:** Only PDFs generated by the Podman/TeX Live job are uploaded and committed back to the repository. Windows PDFs are temporary verification output and are discarded with the runner.
