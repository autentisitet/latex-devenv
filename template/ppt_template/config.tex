...
\AtBeginDocument{
  \iftoggle{fontBlackTheme}{
    % Sans-serif Theme: Optimized for Microsoft YaHei or SimHei
    % Fallback to Fandol system fonts if local fonts are missing
    \setCJKmainfont{SimHei}[AutoFakeBold=true, Fallback=FandolHei-Regular.otf]
    \setmainfont{Arial}
  }{
    % Serif Theme: Optimized for SimSun and Times New Roman
    \setCJKmainfont{SimSun}[AutoFakeBold=true, Fallback=FandolSong-Regular.otf]
...
% Equation Highlight Box: Auto-scaling TikZ node with background fill
\pgfkeys{/BehindEqBox/.is family, /BehindEqBox,
  color/.initial = midblue, border/.initial = 1pt, padding/.initial = 2.8mm}
  ...
  \begin{center}
    ...
    \begin{tikzpicture}
      ...
      \begin{scope}[on background layer]
        \filldraw[fill=\pgfkeysvalueof{/BehindEqBox/color}, fill opacity=0.1, rounded corners=0.15cm,
                  draw=\pgfkeysvalueof{/BehindEqBox/color}, draw opacity=0.3,
                  line width=\pgfkeysvalueof{/BehindEqBox/border}]
          ([xshift=-12pt, yshift=-2pt]E.south west) rectangle ([xshift=12pt, yshift=2pt]E.north east);
      \end{scope}
    \end{tikzpicture}
    \end{adjustbox}
  \end{center}
